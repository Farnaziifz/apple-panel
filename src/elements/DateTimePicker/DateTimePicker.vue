  ...
  <div class="mnx-dtp" :style="theme">
    ...
    >
      ...
      <div class="mnx-dtp-header" v-if="!range">
        <div class="year-container">{{ this.selectedDateArray[0] }}</div>
        ...
        <div class="double-month-container" v-else>
          <div class="first-month">
            ...
            <div class="days-container">
              ...
              >
                <div class="day-cell">{{ day.date[2] }}</div>
              </div>
            </div>
          </div>
      ...
      <div class="action-bar-container">
        <button
          class="confirm"
          :class="{ disable: !isConfirmButtonActive }"
          @click="
            () => {
              this.isOpen = false;
...
export default {
  ...
  methods: {
    ...
    // emit data of ranges
    emitDateRanges() {
      const firstDate = this.convertToEnglishNumber(
        new PersianDate(this.rangeDateArray[0].date).format(this.format)
      );
      const secondDate = this.convertToEnglishNumber(
        new PersianDate(this.rangeDateArray[1].date).format(this.format)
      );
      const emittedDates = firstDate + " - " + secondDate;

      this.$emit("input", emittedDates);
      this.$emit("onDateSelected", [firstDate, secondDate]);
    },
    emitSingleDate() {
